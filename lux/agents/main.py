...
from kit.kit import from_json
from kit.logger import log_actions
...
def agent_fn(Agent):
    ...
    """
    def fn(observation, configurations):
        global agent_dict
        obs = observation.obs
        if type(obs) == str:
            obs = json.loads(obs)
        ...
        log_actions(f'actions_{agent.team_id}.json', agent, obs, actions, remainingOverageTime)
